...
class DenoisingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # noisy_path looks like: .../noisy/NORMAL_img_0_noise_10.png
        # Clean equivalent is: .../clean/NORMAL_img_0.png
        filename = os.path.basename(noisy_path)
        # remove the _noise_XX part to get clean filename
        # Split by "_noise_"
...
def train_model(dataset_dir, epochs=10, batch_size=32, lr=1e-3):
    ...
    model_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "models", "unet_denoiser_model.pth")
...
if __name__ == "__main__":
    DATASET_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "poisson_dataset_2")
    
    # Train the model with more epochs since dataset size is smaller
    model, val_loader = train_model(DATASET_DIR, epochs=50, batch_size=16)
